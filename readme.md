...
实现多格式文档上传与解析模块，支持 PDF、Word(docx)、TXT、Markdown 等文件类型；后端基于文件类型自动调用 pypdf、python-docx 或纯文本解析器，将不同格式文档统一转化为文本后进行 chunk 切分、向量化入库和元数据持久化，提升 RAG 系统对真实业务文档的适配能力。
